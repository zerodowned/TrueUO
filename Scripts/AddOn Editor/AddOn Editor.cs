	...
	public class AddOnEditor : Gump
	{
		...
		public AddOnEditor( Mobile from, AddOnEditor_Att att ) : base( 100, 100 )
		{
			...
			if( att.SelectedAddon != null )
				 TypeName = att.SelectedAddon.GetType().Name; //.ToString();
			...
			AddHtml( 34, 160, 165, 17, @"<BASEFONT COLOR=#FFFFFF><center>Change to Static Item", (bool)false, (bool)false);
		...
		private class Add_Target : Target
        {
			...
			protected override void OnTarget(Mobile from, object targ)
			{
				...
				if( targ is AddonComponent ) 
				{
					...
					{
						from.SendMessage("You cannot add two addons together");
						from.Target = new Add_Target(from);
					}

					
				}
				if( targ is Item targItem)
				{
					BaseAddon addon = addoneditor.SelectedAddon;

					addon.AddComponent(new AddonComponent(targItem.ItemID),  targItem.Location.X - addon.X , targItem.Location.Y - addon.Y, addon.Z);
					targItem.Delete();
				}
				else 
				{
		...
		private class Remove_Target : Target
        {
			...
			protected override void OnTarget(Mobile from, object targ)
			{
				...
				if( targ is AddonComponent ) 
				{
					...
					{
						Server.Items.Static staticItem = new Server.Items.Static(component.ItemID);
						staticItem.MoveToWorld(component.Location, component.Map);
						component.Delete();
					}
				}
				else 
